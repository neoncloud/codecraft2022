...
class Robot:
    ...
    def get_action(self, adj_mat: np.ndarray, headin_glist: np.ndarray, robot_coord: np.ndarray):
        ...
        def potential_field(self, task_index: int):
            ...
            for obs in obstacles:
                ...
                if obs_d <= obs_rep_range:
                    ...
                    obs_rep += obs_k_rep * \
                        (1/obs_d - 1/obs_rep_range) * \
                        (1/obs_d**2) * obs_vec / obs_d
            ...
            distance = np.linalg.norm(
                self.task_coord[task_index, :]-self.coord, axis=-1)
        ...
        def velocity_obstacle(self, task_index: int):
            ...
            for robot in other_robots:
                ...
                if obs_distance > VO_distance_range:
                    continue
                obs_dir = np.arctan2(
                    obs_relative_pos[1], obs_relative_pos[0])  # [-pi,pi]
                if obs_dir < 0:
                    obs_dir = 2*np.pi + obs_dir  # [0,2pi]
            ...
            tar_relative_pos = np.float64(
                self.task_coord[task_index, :] - self.coord)
            tar_distance = np.linalg.norm(tar_relative_pos)
            tar_dir = np.arctan2(
                tar_relative_pos[1], tar_relative_pos[0])  # [-pi,pi]
            if tar_dir < 0:
                tar_dir = 2*np.pi + tar_dir  # [0,2pi]
            ...
            dis_two_tar = np.linalg.norm(np.float64(
                self.task_coord[0, :] - self.task_coord[1, :]))
            if dis_two_tar > 20:
                temp = 4
            elif dis_two_tar > 10:
                temp = 2
        ...
        def v_obstacle(self, task_index):
            ...
            w_delta_time = 0.02
            desired_velocity = (
                self.task_coord[task_index, :] - self.coord) / v_delta_time
            ...
            # Calculate the velocity obstacles
            other_robots = np.delete(robot_coord, self.index, axis=0)
            velocity_obstacles = [calculate_velocity_obstacle(
                self, other_robot) for other_robot in other_robots]
            ...
            for angle in np.linspace(0, 2*np.pi, 180):
                # print('angle=',angle)
                available = True
                candidate_orientation = np.arctan2(
                    desired_velocity[1], desired_velocity[0]) - angle
                for vo in velocity_obstacles:
                    if vo[0] < candidate_orientation and candidate_orientation < vo[1]:
                        available = False
                        break
        ...
        def final_version(self, task_index):
            ...
            for angle in np.linspace(0, 2*np.pi, 90):
                ...
                candidate_orientation = (
                    target_orientation - angle) % (2*np.pi)
                for vo in velocity_obstacles:
                    if vo[0] < candidate_orientation and candidate_orientation < vo[1]:
                        available = False
                        break
...
class Map:
    def __init__(self, frame_num: int, money: int, workbenches: List[Workbench], robots: List[Robot]):
        ...
        self.robot_v = np.array([r.linear_velocity for r in self.robots])
    ...
    def output(self):
        ...
        for i, r in enumerate(self.robots):
            sell, buy, destroy, w, v = r.get_action(
                self.robot_adj_mat, self.robot_heading, self.robot_coord)
