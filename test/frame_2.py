...
class Workbench:
    def __init__(self, type_id: int, x: float, y: float, remaining_time: int, material_state: int, product_state: bool, index: int):
        ...
        self.incoming = []
        self.product_done = False

    def update(self, remaining_time: int = None, material_state: int = None, product_state: bool = None, robot_coord:np.ndarray=None):
        # 重要！只有当：之前没产出，更新后产出了，才等于成功产出一个，不重不漏
        # 生产出来了一个东西，就加入排单队列
        min_time = np.linalg.norm(
            robot_coord - self.coord, 2, -1).min(0)/6.0
        if self.type_id in (1, 2, 3) or ((product_state or (remaining_time/50.0 <= min_time and remaining_time>0)) and not self.product_state):
    ...
    def update(self, input_string: str):
        ...
        for i, w in enumerate(self.workbenches, 2):
            ...
            w.update(int(workbench_data[3]), int(
                workbench_data[4]), bool(workbench_data[5]), self.robot_coord)
